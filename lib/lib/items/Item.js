...
var Item = function (_Component) {
  ...
  _createClass(Item, [{
    ...
    value: function render() {
      ...
      if(this.props.item.id === 'jceovrrpba') {
        // console.log(quantity_milestones)
      }

      if(focus === 'day' || focus === 'week') {
        if (this.props.item.group !== 'milestones') {
          if(quantity_milestones > 1) {
            top = top - (100 * quantity_milestones) - (50 * (quantity_milestones - 1));
            
            if(this.props.item.id === 'jceovrrpba') {
              // console.log('on the if', top)
            }
      ...
      if(_temp === 30) {
        if (this.props.item.group !== 'milestones') {
          top = dimensions.top - (30 * (quantity_milestones - 1));
          // top = dimensions.top;
        } else {
          top = 60;
        }
