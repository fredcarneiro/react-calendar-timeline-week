...
var Items = function (_Component) {
  ...
  _createClass(Items, [{
    ...
    value: function render() {
      ...
      var _visibleItems = visibleItems.filter(function (item) {
        return sortedDimensionItems[(0, _utils._get)(item, itemIdKey)];
      });

      var _milestones = _visibleItems.filter(function(item) {
        return item.craft === 'milestones'
      });

      var _milestonesGroups = { };
      _milestones.forEach(function(item){
        var list = _milestonesGroups[item.start_time];

        if(list){
            list.push(item);
        } else{
            _milestonesGroups[item.start_time] = [item];
        }
      });

      var _milestonesGroupsKeys = Object.keys(_milestonesGroups);

      var _maxMilestonesQuantity = 0;

      _milestonesGroupsKeys.forEach(function(key) {
        var quantity = _milestonesGroups[key].length;

        if(quantity > _maxMilestonesQuantity) _maxMilestonesQuantity = quantity;
      });

      //_maxMilestonesQuantity = _milestones.length;
      ...
      return _react2.default.createElement(
        ...
        visibleItems.filter(function (item) {
          ...
          return _react2.default.createElement(_Item2.default, { key: (0, _utils._get)(item, itemIdKey),
            ...
            _milestones: _maxMilestonesQuantity,
            focus: focus
          });
        })
      );
