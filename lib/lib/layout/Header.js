  ...
  _createClass(Header, [{
    ...
    value: function headerLabel(time, unit, width) {
      ...
      if (unit === 'year') {
        ...
        return "KW "+time.format(f.week);
      } else if (unit === 'day') {
        // return time.format(width < 150 ? f.dayShort : f.dayLong);
        // console.log('headerLabel', width);  
        return time.format(width < 170 ? 'dddd, DD MMM' : 'dddd, DD MMMM');     
      } else if (unit === 'hour') {
    ...
    value: function subHeaderLabel(time, unit, width) {
      ...
      if (unit === 'year') {
        ...
        // console.log('subHeaderLabel', width);        
        return time.format(width < 170 ? 'dddd, DD MMM' : 'dddd, DD MMMM');
      } else if (unit === 'hour') {
    ...
    value: function render() {
      ...
      if (twoHeaders) {
        (function () {
          ...
          (0, _utils.iterateTimes)(visibleTimeStart, visibleTimeEnd, nextUnit, timeSteps, function (time, nextTime) {
            ...
            var moment = _props.frozen;
            
            var isFrozen = '';

            if (nextUnit === 'week' && moment !== false || nextUnit === 'day' && moment !== false) {
              if (moment === 'any' || time.format('x') < moment.format('x')) {
                isFrozen = 'rct-frozen' + ' ';
              }
            }
            ...
            timeLabels.push(_react2.default.createElement(
              ...
              { key: 'top-label-' + time.valueOf(),
                href: '#',
                className: 'rct-label-group' + (hasRightSidebar ? ' rct-has-right-sidebar' : '') + ' ' + isFrozen,
                'data-time': time,
                'data-unit': nextUnit,
                style: {
      ...
      (0, _utils.iterateTimes)(canvasTimeStart, canvasTimeEnd, minUnit, timeSteps, function (time, nextTime) {
        ...
        if (minUnit === 'week' && moment !== false || minUnit === 'day' && moment !== false) {
          if (moment === 'any' || time.format('x') < moment.format('x')) {
            isFrozen = 'rct-frozen' + ' ';
          }
        }
        ...
        timeLabels.push(_react2.default.createElement(
          ...
          { key: 'label-' + time.valueOf(),
            href: '#',
            className: 'rct-label ' + (twoHeaders ? '' : 'rct-label-only') + ' ' + (firstOfType ? 'rct-first-of-type' : '') + ' ' + (minUnit !== 'month' ? 'rct-day-' + time.day() : '') + ' ' + isFrozen,
            'data-time': time,
            'data-unit': minUnit,
            style: {
